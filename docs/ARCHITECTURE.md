...
DailyProfile 14.0、WeeklyProfile 6.0、MonthlyProfile 3.0 和 Agent Context 6.0
...
DailyProfile 14.0 通过 Running Analysis v2 嵌入 `TrainingFeatures.running`。
...
明确的厂商组数据和用户确认的动作是精确动作身份的唯一来源。有效 `strength_sets` 观测优先，
不与回退观测重复叠加。仅当 `training_family=strength` 且 lap 行恰为 62 列时，从 0-based 21、22、
28 读取重量原数值、正整数次数和正整数 `vendor_exercise_code`，并保留 `order`。观测使用
`source`=`strength_sets` 或 `lap_62`，以及 `vendor_exercise_code`、`weight_value`、`weight_unit`、
`limitations`；`lap_62` 进入独立有序的 `observed_sets`，不进入 `explicit_exercises`，也不抬高动作覆盖、
...
DailyProfile 14.0 将带时间戳的普通心率与每日指标序列分开保存。对于每个睡眠区间，
...
schema_version=4.0` 分开展示昨夜睡眠与身体读数、昨天已记录的活动及训练、今天截至分析时的活动；
每条数值注明日期、单位和必要的来源。若前七天训练来源尚未查全，事实报告仍展示已观测数据，
但不生成今天的训练剂量；当天尚未有 workout 也不是缺项。只有决策所需信号不足才会返回
`INSUFFICIENT_DATA`，晨报处方不混入历史 `observed_sets`。未证实的训练缺席、不同设备数值的
合并、推测动作与通用训练模板都不能进入每日推送，真实的安全门控仍在结构化档案中执行。

晚间渲染器是独立的确定性视图。它会在有数据时按 `started_at` 报告当天每场实际训练
明细，包括跑步指标以及明确的力量训练组或有序观测组；确认记录优先，逐组展示 `observed_sets`，
缺失和未知单位保持缺失，不补写 `kg` 或动作名称。它还汇总融合后的每日活动和设备记录的压力，解释最近已完成日期的训练负荷，并以恢复及

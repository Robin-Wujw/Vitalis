...
活动字段的缺失保持为 `None`。`ActivityRecord` 的 `observed_fields` 只记录载荷中明确出现且有效的字段；旧 JSON 中没有观测证据的默认 `0` 不作为真实测量，旧正数可保留但带有兼容语义。步骤、距离、活动分钟和热量会进入独立来源流，距离仅在单位已知时转换为 `km`。

泛称 calories 的能量角色保持为 `unspecified`，不会被称为总能耗；ActivityRecord、DailyMetric 和 workout 等重复入口不相加，训练热量不并入日总。没有摄入数据时不判断热量赤字。不同 `source_scope`、设备和单位保持独立。
...
Zepp App `10.8.7-play` 的逐组解析将 `lap_62` 第 28 列写入 `strengthTrainType`，展示层再用它匹配区域接口 `GET /sport/config/muscle` 返回的 `actionType`，读取中文目录中的 `actionName`。以请求头 `lang=zh_CN` 及查询参数 `lang=zh_CN&country=CN&LANGUAGE=zh_CN` 获取的中文目录响应 SHA256 为 `7e6b4ba617bd8e61b0c269c9c51fb4d7b430c0e30332273a964fa88f8028abeb`；既有 8 个代码的动作语义均吻合，其中 `1770` 的旧“颈前”显示拼写按厂商和用户原文“劲前”修正。`ZEPP_STRENGTH_LAP_LABELS` 只保留已观测代码，不复制整个厂商目录；这些名称仅供展示，不等同于用户确认的动作、重量单位或训练处方证据。未来未见代码保持未知，凭据不会写入 URL、日志或仓库。

离线核查工件由用户提供且未纳入仓库：包名 `com.huami.watch.hmwatchmanager`，版本 `10.8.7-play`，versionCode `151975`，APKMirror [Zepp 索引页](https://www.apkmirror.com/apk/zepp-inc/amazfit-watch/)（并非该文件的精确下载 URL），`.apkm` 容器 SHA256 为 `64e1d87b6ab79e0ecedfbead176cc6bb781f9754b46915d52c0d2ccfbcef5949`。容器包含 `base.apk` 与 2 个架构拆分包；本轮仅离线检查 `base.apk` 的资源/DEX，不运行 App、不提交原安装包；复核前先比对指纹，再按上段的字段和目录路径核对编码空间。

历史训练明细不会因同步较长日期范围而自动全部刷新：通常只在近 28 天内每次最多补四份。维护人员确需补查旧训练时，备份后显式运行 `python skills/vitalis/tools/sync.py --days 730 --workout-only`（天数按可取历史缩短；此命令会访问 Zepp 并写入本地数据库）。该选项仅用于手动同步：完整分页检查所选窗口的全运动汇总源，并从窗口内最多补四份缺失、旧版或待刷新的训练明细，不重复请求睡眠、心率等健康流。普通手动健康同步需要按窗口补明细时可单独使用 `--detail-backfill`。已完成一次全运动分页核查后，可用 `python skills/vitalis/tools/sync.py --days 730 --details-only` 仅将库内最多四份缺失或旧版明细入队，不重复获取运动概要，也不改变来源覆盖；待办为空时返回 `no_pending_details`。低内存试验可显式运行 `python skills/vitalis/tools/sync.py --days 730 --details-only --detail-limit 1`，将持久清单固定为一份；不指定时仍为四份。单份请求还受 4 MiB 响应、20,000 条估算/实存样本上限及 worker `MemorySwapMax=128M` 保护，超限拒绝且保留缺口，绝不截断后标为成功。只有需要重取已有当前版本明细（例如新核验的动作名）时才附加固定 UTC 截止值 `--refresh-before <utc-iso>`，确保重复批次能够收敛。应检查同步尝试进度与明细缺口，必要时分批继续，不能把单次成功等同于所有历史详情已齐。低内存 SQLite 部署不应连续高速运行 `--details-only`：旧训练详情和类型化样本可能让写事务、swap 与磁盘 I/O 长时间拥塞。应在维护窗口或资源充足的环境分批执行并监控负载；遇到持续写锁或无进展先停止补采、恢复服务，并如实保留缺口。`python -m vitalis.workout_audit --database <db-file> --user <user-id>` 可只读统计已保存的训练、明细版本、云端运动类型代码和力量动作代码，并指出当前已核验代码仍缺名称的旧记录；其 `source_coverage=NOT_ASSESSED` 表示此工具不代替同步账本证明云端覆盖。

已保存的当前版明细若只缺后来核验的动作显示名，先运行 `python -m vitalis.strength_label_refresh --database <db-file> --user <user-id>` 只读预览更新范围；可用 `--code <verified-code>` 限定代码。确认备份后才附加 `--apply --backup <verified-backup>`。该操作只填补 `lap_62` 中名称为空且标为未核实的已核验代码；唯一的非空名称例外是代码 `1770` 本项目旧版“颈前”与厂商目录“劲前”的精确拼写修正，且必须带参考映射标记。其它已有名称不覆盖；不请求厂商、不改组次/负重/来源覆盖，也不自动更新已有分析快照。不得直接对早于当前活跃健康事件评估日的历史日期调用 `POST /intelligence/analyze` 补标签：现有生命周期可能倒序冲突，已观察到 500/`FAILED`。旧快照保持不变，待安全隔离重建；正常当前及未来分析会读取已更新的明细。
...
真实的可选数据流或密集文件网络/身份验证故障会保留为失败，绝不会报告为空账户。同步和投递的 timeout/5xx 属于可重试故障；明确的 `reauth` 会阻断继续投递。身份验证失败会将浏览器链接标记为需要登录；暂时性故障会保持连接并可重试。`partial` 按数据流领域判断，不能由其他领域的成功掩盖。

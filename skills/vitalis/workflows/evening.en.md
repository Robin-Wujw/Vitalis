...
2. When returned by `features.training.running` or `features.training.strength`, show the available discipline-specific running metrics, strength sets, and confirmed exercises. Give confirmed records precedence, then show `observed_sets` one set at a time in `order`. Preserve the `source` literals `strength_sets` / `lap_62`, `vendor_exercise_code`, `weight_value`, `weight_unit`, and `limitations`; without a verified exercise dictionary show only the code, preserve nulls and unknown units, and do not invent `kg`, exercise names, or weights. Negative sentinels remain null.

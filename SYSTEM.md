...
- 2026-09-26 `de722b3` 已部署并验收：Zepp App `10.8.7-play` 的逐组第 28 列经代码路径证明对应区域中文目录的 `actionType` / `actionName`，全部既有核验锚点吻合；只把本库已观测的代码显示名加入解析器，不提交 APK 或完整厂商目录。本地及服务器完整 Python 测试各 823 项通过，备份和 schema 审计通过；用户审阅精确 dry-run 后仅修正已有明细的空白名称及一项旧拼写，后续只读审计显示已保存动作代码都有名称，来源覆盖和原始训练数值不变。旧训练明细补采仍因低内存暂停。历史日期倒序重算在第一天因事件生命周期校验失败而停止，用户选择保留旧快照；今日正常分析质量充分、前七天覆盖已核实。未额外发送测试推送。
...
- 本轮字段可用性检查不展开个人健康数值；可以描述一般能力，但不把个人健康数值、真实记录或未取得的 App 修正动作写入仓库。已验证的厂商动作目录不等于用户在 App 中修正后的逐组记录。
...
- [ ] `AUD-017` **P2 / 力量动作证据**。已从 Zepp App 实际使用的中文目录验证并映射当前已保存的动作代码，但不复制完整目录；App 修正组来源仍未确认，低内存服务器的旧训练明细补采因写锁暂停。未来未见代码不能推断名称，`unit` 未知时不补写，未取得详情的动作 `name` 仍未知。完成：在资源允许的维护窗口补齐可取明细，验证 App 修正记录来源与优先级，并为未来新代码建立可核对的目录更新路径。
...
- [ ] `AUD-020` **P2 / 历史重算事件隔离**。对较早日期执行 `POST /intelligence/analyze` 可能将较晚的活跃健康事件倒序协调，产生 `duration_days` 小于 1 的无效结果；已见请求返回 500，运行标为 `FAILED`。用户选择保留旧快照，当前不可直接批量倒序重算。完成：隔离历史事件生命周期的投影或提供明确拒绝，验证不会篡改当前事件、训练判断和既有快照后再开放安全重建。

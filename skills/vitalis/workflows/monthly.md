...
3. `sleep_recovery` 和 `training_activity`：各优先展示 1–2 条有代表性的 `facts`，保留原始单位、有效天数和前期对比分母；详细数值只在需要时展开。“已记录小计”不是完整 28 日总量。不同设备流有冲突时分别显示，缺失不写成零，未知日不写成休息日。不同口径热量不相加，也不推断热量赤字；设备评分不作为月度结论。

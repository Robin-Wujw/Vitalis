...
3. `sleep_recovery` and `training_activity`: Preserve units, valid-day counts, and prior-period denominators beside returned `facts`. A recorded subtotal is not a full 28-day total. Separate device streams, never turn missing values into zero or unknown days into rest days, and never add unlike energy scopes or infer an energy deficit.

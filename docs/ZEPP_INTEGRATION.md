...
已保存的当前版明细若只缺后来核验的动作显示名，先运行 `python -m vitalis.strength_label_refresh --database <db-file> --user <user-id>` 只读预览更新范围；可用 `--code <verified-code>` 限定代码。确认备份后才附加 `--apply --backup <verified-backup>`。该操作只填补 `lap_62` 中名称为空且标为未核实的已核验代码；唯一的非空名称例外是代码 `1770` 本项目旧版“颈前”与厂商目录“劲前”的精确拼写修正，且必须带参考映射标记。其它已有名称不覆盖；不请求厂商、不改组次/负重/来源覆盖，也不自动更新已有分析快照；需要另外重算相关日期。

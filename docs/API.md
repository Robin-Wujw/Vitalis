...
| GET | `/intelligence/context?day=YYYY-MM-DD` | 读取 Agent Context 6.0 的有界 Current/Recent/Trend/Personal 智能体上下文 |
...
| POST | `/health/sync?days=&decode_dense_files=false&detail_backfill=false&workout_only=false&detail_only=false&detail_limit=` | 同步健康数据；密集归档默认只建索引。手动 `workout_only=true` 仅获取全运动历史及最多四份明细；手动 `detail_backfill=true` 在普通同步窗口内补抓旧明细；`detail_only=true&enqueue_only=true` 默认最多四份，显式 `detail_limit=1` 将单个持久任务固定为一份，不证明运动历史覆盖，待办为空时返回 `no_pending_details` |
...
`retry_wait` 保留已完成的分块和下一次重试时间戳；timeout 与 5xx 属于可重试故障，
`needs_reauth` 仅用于已分类的身份认证拒绝并阻断继续投递；`partial` 按数据流领域判断，
包括成功/不可用覆盖混合的情形或调用方明确指定的截止时间。CLI 与内置投递入口共用
用户/日期锁标记。取消操作具有幂等性，并且在进程重启后仍然有效。手动请求在返回前最多推进

...
全程使用普通中文，按 `sections` 顺序展示已返回的昨晚睡眠、今早身体信号、昨天活动与已记录训练、今天截至分析时的活动；有依据的今天安排放在最后。每部分先选最重要的 1–2 条可验证事实及对应结论，保留日期、单位和有意义的来源差异；不逐项堆叠采样量、设备评分或重复的“Zepp 汇总”。详细事实仍以原 `sections` 为准，用户询问时再展开，不自行推算。`observations`、`key_reasons` 与章节重复时不再复述。当天尚无 workout 不代表数据缺失，不得因此变为 `INSUFFICIENT_DATA`；晨报处方不得混入历史 `observed_sets`。

事实版（`facts_only`）也可展示按日期核对的活动和已记录训练事实；已观测的力量动作按连续同名组概述，与用户确认的动作分开，不把已记录场次说成完整训练历史；昨天的活动不替代今天的记录。训练历史未查全时只说明一次，不得补写今天的训练动作、强度、重量或从完整日报重建处方。完整报告按以下顺序渲染：

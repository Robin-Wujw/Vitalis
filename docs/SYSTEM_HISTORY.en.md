# Vitalis Development System Historical Archive
...
## 2026-09-26 Zepp Exercise Catalog Verification and Retrospective Boundary

- The user-provided Zepp App `10.8.7-play` package was inspected offline, never run or committed. DEX evidence connects column 28 of `lap_62` to `strengthTrainType`, then to `actionType` and `actionName` from the regional `GET /sport/config/muscle` catalog. Every preexisting name anchor matched the Chinese response. `de722b3` stores display labels only for codes already observed in this database, not the full vendor catalog, and does not promote them into prescription evidence. All 823 local and server Python tests passed; the pre-deployment backup and schema audit were clean.
- After the user reviewed the dry run and each affected row was matched against its backup, blank stored exercise names and one earlier spelling were refreshed in a small transaction. Read-only checks show names for all currently stored codes, with source coverage and original weight/repetition values unchanged; historical missing details remain explicit. Database `quick_check=ok` and foreign-key checks passed, API/worker remained active, and no extra test push was sent.
- Bulk backdated analysis failed on its first day with `HealthEvent.duration_days` validation, HTTP 500, and a `FAILED` analysis run, then stopped immediately. Current active events cannot safely be replayed backward; the user chose to preserve old snapshots. The failed transaction did not proceed to other historical days, while normal current-day analysis remained `SUFFICIENT` with `prior_7d_verified=true`. Safe retrospective reconstruction is tracked as `AUD-020`.

## 2026-09-25 Account-wide Workout Proof and Report Recovery

- `AUD-009` is closed: after verifying Zepp's `/run/history.json` as the account-wide mixed-workout feed, `zepp-sync-v6` requires every successful page from that feed to prove workout-source coverage. Old attempts retain their original partition requirements; vendor 404 responses, unfinished pages, and incomplete local days do not count as complete. A real sync verified the prior seven Morning days at 7/7 with `prior_7d_verified=true`, so the obsolete 13-partition rule no longer suppresses the training plan.
- Before deploying `af4a653`, backups were created and verified; schema/identity audits, all 800 local and server Python tests, 6 local Balance 2 Node tests, and 47 bilingual checks passed. After an SQLite write lock, the worker recovered through cancellation of the exact orphaned attempt and a restart; `quick_check=ok` and foreign-key checks passed. The existing scheduled Morning retry enqueued successfully and delivered with its normal deduplication marker; no extra test push was sent.
- Historical workout details remain incomplete: inspected cloud `strengthSets` did not provide a complete movement-name catalog, and bulk detail fetching on the low-memory host (1.6 GiB RAM, 1 GiB swap) caused memory reclaim and a write lock. Remaining backfill was paused with explicit gaps. `AUD-017` remains open; unknown exercise codes, weight units, and unavailable app-corrected names are not presented as verified.
- `c327ac0` adds a read-only preview and backup-original-verified refresh of `lap_62` display labels. Existing limited mappings trace to an App screenshot and ordered cloud-lap comparison; newer mappings trace to a user-reported same-session movement order. Neither is an official complete dictionary. After the user reviewed the exact scope, production updated only blank names for verified codes; related snapshots were recomputed and report display verified. Unknown actions and prescription evidence remain unchanged. All 804 local Python tests and 114 focused server tests passed; final database `quick_check=ok` and foreign-key checks passed, with no extra push.
...
- After rehearsing on two complete database copies, rebuilt 16 drifted tables in the local database in two stages. Counts for 1,128,976 metric samples, 346,510 training samples, analyses, recommendations, stream states, and connection records were all preserved; old `NULL device_id` values were normalized to empty strings under the current contract.
- After migration, schema/identity audits were clean, `quick_check=ok`, foreign-key errors were 0, and `init_db()` succeeded. Pre-migration backups separately covered the column/state tables and natural-key constraint tables.
- The UTC-day request for `all_day_stress` now extends by one day on each side and is trimmed back to the configured timezone's original half-open window before persistence; the durable plan was upgraded to `zepp-sync-v4`.
- The final state of a real 2026-09-03 synchronization was `partial` only because optional capabilities were unavailable; the stress stream completed fetch/parse/write successfully with 234 local-day points in the range 5-65. Analysis returned `SUFFICIENT` / `TRAIN_NORMAL`, and Morning/Evening projections succeeded; stress did not enter decision evidence.
- 8 schema migration unit tests, 101 focused synchronization/schema tests, and all 425 tests passed; compileall, 46 OpenAPI paths, Markdown links, and `git diff --check` passed.
- Before server deployment, a SQLite backup was created and passed `quick_check`; all 425 server tests passed, schema/identity audits were clean, API/worker were active, `healthz=ok`, and the post-deployment error-log count was 0.
- The real server synchronization ended `partial`: among 31 chunks, 12 optional chunks were unavailable and 0 failed; analysis returned `SUFFICIENT` / `TRAIN_NORMAL`, with 234 stress points in the range 5-65. The Morning PushPlus test send succeeded without changing the formal schedule marker.

## 2026-09-04 Zepp Identity Migration Hardening

- Ordinary `upsert_user()` no longer overwrites a source projection when the new identity is absent, preventing pairing, analysis, or link creation from destroying an existing connector identity.
- `save_token()` normalizes vendor IDs at the repository boundary; the Zepp projection remains unique, while tokens for other connectors continue to be stored independently by `(user_id, source)`.
- The audit adds `orphan_projections` and provides an explicit `clear-projection` command; the resolver checks cross-user token/projection ownership before deleting a token, audits again after migration, and rolls back if the result is not clean.
- Startup audits first and then reuses the unified unique-index creation function; the real local database audit was clean and `init_db()` succeeded.
- 76 focused identity and API tests passed; all 417 tests passed, along with compileall, 46 OpenAPI paths, and `git diff --check`.

## 2026-09-04 Zepp Stress Time Series

- Semantics were checked using the same Zepp account, the same device, and matching UI dates against a redacted payload: daily average, minimum, maximum, and four-band percentages come from the `all_day_stress` daily summary, while the chart comes from its `data` JSON time points.
- `parse_wellness()` now stores explicitly timestamped 0-100 vendor values as device-scoped `MetricSample(metric="stress")` records; gaps remain missing, band thresholds are not derived, and stress does not enter recovery or training decisions.
- `Charge/real_data` continues to represent only hybrid/physical/mental Charge; the `Charge/stress_data` protobuf and `Charge/insight_data` types still have no provable semantics and remain disabled.
- The focused parser, sync-manager, health-API, and fetcher regressions passed 140 tests; all 410 tests passed, along with compileall, 46 OpenAPI paths, Markdown links, and `git diff --check`.
- Real coordinator verification was incomplete: the old local database lacked `sync_attempts.trigger_ref`, so creating a synchronization attempt failed as a schema mismatch. That blocker remained in `SYSTEM.md` and was not described as a parser failure or an end-to-end success.

## 2026-09-04 Chinese and English Documentation Entry Points

- Reduced the root `README.md` to a Chinese product entry point containing only product purpose, the data-to-action path, core capabilities, trust boundaries, current status, and primary documentation links.
- Added the equivalent English entry point `README.en.md` without duplicating installation, deployment, API, or algorithm details.
- Added the bilingual `docs/README.md`, organized by four reader tasks—usage and operations, API/Agent integration, architecture and research, and project maintenance—and made each document's sole maintenance responsibility explicit.
- Verification: every local Markdown link in the three entry-point files existed and `git diff --check` passed; this documentation-only change did not rerun the 404 Python tests that had already passed.

## 2026-09-04 Zepp Identity Ownership

- Added database unique indexes for `users(source, source_user_id)`, `auth_tokens(source, source_user_id)`, and `auth_tokens(user_id, source)`.
- Changed token storage to an atomic identity claim; ownership conflicts from manual import, initial pairing, and browser renewal now uniformly return HTTP 409.
- A real Zepp import must provide the vendor `userid`, and the synchronization client no longer falls back to a potentially stale user projection.
- Added read-only audit, explicit canonical resolution, and migration commands. A non-canonical user loses only conflicting credentials and browser links; health, training, analysis, and feedback history is neither merged nor deleted.
- `delete_for_user()` now completely deletes tokens, OAuth state, and user mappings so that explicit full cleanup truly releases the vendor identity.
- Verification: 12 focused migration-boundary and callback-conflict tests passed; the complete `python -m pytest -q` run passed 404 tests; compileall, OpenAPI generation, and `git diff --check` passed.
- Before local old-database migration, the backup passed SQLite `quick_check`; `001` was then selected as canonical and the conflicting token for `real001` was revoked. After migration there were no audit duplicates, all unique indexes were present, real `init_db()` succeeded, and health-history counts for both users remained unchanged.

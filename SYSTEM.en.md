...
- On 2026-09-26, `de722b3` was deployed and accepted: the Zepp App `10.8.7-play` code path proves lap column 28 maps to the regional Chinese catalog's `actionType` / `actionName`, and every previously verified anchor matches. Only labels for codes observed in this database entered the parser; neither the APK nor the full vendor catalog was committed. All 823 local and server Python tests passed, along with backup and schema checks. After the user reviewed the exact dry run, only blank names and one earlier spelling were updated in stored details; read-only audit now finds names for all stored exercise codes, with source coverage and original training values unchanged. Old workout-detail backfill remains paused on the low-memory host. Out-of-order historical reanalysis failed on the first day due to event-lifecycle validation, and the user chose to preserve old snapshots; today's normal analysis has sufficient quality and verified prior-seven-day coverage. No extra test push was sent.
- On 2026-09-25, `c327ac0` was deployed and accepted: all 804 local Python tests and 114 focused server label/bilingual tests passed. After backup validation and a matching row-by-row dry run, only blank display names for verified codes in stored details were refreshed. The explicitly authorized affected-date snapshots were recomputed, their Evening report displayed the names, and today's prior-seven-day coverage did not regress. Database `quick_check=ok` and foreign-key checks passed. Unknown exercises remain unknown, old-detail backfill stays paused, and no extra test push was sent.
- On 2026-09-25, `af4a653` was deployed on the server: backups, schema/identity audits, and SQLite integrity/foreign-key checks passed; all 800 local and server Python tests, 6 local Balance 2 Node tests, and 47 bilingual checks passed. Historical pages from the account-wide Zepp workout feed were fetched successfully, and the `zepp-sync-v6` ledger proved the prior seven days complete. After resolving a worker-held write lock, today's scheduled Morning report was delivered with its normal deduplication marker and `prior_7d_verified=true`; no additional test push was sent. Bulk refresh of old workout details is paused because of server memory and swap pressure; the exact orphaned attempt was cancelled and the database rechecked clean. Unverified exercise codes and app-corrected names unavailable from the known vendor detail endpoint remain unknown; neither all details nor a complete exercise dictionary is claimed.
- On 2026-09-24, `35a9d93` was pushed and deployed: the pre-upgrade backup, schema/identity audits, compilation, three running services and liveness check passed. All 736 local Python tests and 6 Balance 2 Node tests passed; full pytest was not run on the server. An explicitly authorized analysis created today's new snapshot; a read-only preview covered overnight and body signals, yesterday's activity and recorded training, and today's activity through the cutoff, without a prescription. A real new PushPlus test send was blocked by auto-mode permissions and was not sent by this session; the user had earlier received the old report. Read-only partition inspection found the generic sport endpoint succeeded while 12 categories were unavailable, so observed workouts cannot prove complete history or support a training dose.
...
- Field-availability checks do not expose personal health values. General capabilities may be described, but personal health values, real records, and unavailable app-corrected exercises are not written to the repository. The verified vendor exercise catalog is not the user's app-corrected per-set record.
...
- The current working branch is `fix/zepp-coverage-readable-reports`; English sidecars remain documentation counterparts and do not change runtime report routing.
- Historical baseline from 2026-09-24: Morning scheduling had returned `stored_data_incomplete`: the general history endpoint succeeds while other sport-specific endpoints are unavailable, so the previous seven days lack complete training-history proof. Facts-only delivery does not reinterpret unavailable endpoints as empty records or change history-coverage markers. Server backup and schema audits passed without a structural migration.
...
- [ ] `AUD-017` **P2 / strength exercise evidence**. The Chinese catalog actually used by the Zepp App now verifies labels for exercise codes already stored here; the full catalog is not copied. App-corrected set provenance remains unverified, and old workout-detail backfill on the low-memory server is paused after write-lock pressure. Future unseen codes cannot be inferred; unknown `unit` is not filled in, and missing workout details still have unknown exercise `name`. Done when an adequately resourced maintenance window completes available details, app-correction source precedence is verified, and new codes have an auditable catalog update path.
...
- [ ] `AUD-020` **P2 / retrospective event isolation**. `POST /intelligence/analyze` for an earlier day can reconcile a later active health event backward and produce invalid `duration_days` below 1; an observed request returned 500 and its run was marked `FAILED`. The user chose to preserve old snapshots, so bulk backdated analysis must not be retried as-is. Done when historical event lifecycles have an isolated projection or an explicit rejection, with tests proving current events, training decisions, and stored snapshots cannot be corrupted before safe reconstruction is enabled.

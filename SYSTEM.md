...
- 2026-09-25 `c327ac0` 已部署并验收：本地完整 Python 测试 804 项、服务器标签与双语专项 114 项通过。备份校验与逐条 dry-run 一致后，仅刷新了已核验代码在旧明细中的空白显示名；获授权的相关日期快照重算成功，晚报可见这些动作名，今日前七天覆盖未退化。数据库 `quick_check=ok` 且外键检查通过；未知动作仍未知，旧训练详情补采继续暂停，无额外测试推送。
- 2026-09-25 `af4a653` 已在服务器部署：备份、schema/身份审计和 SQLite 完整性/外键检查通过；本地及服务器完整 Python 测试 800 项、本地 Balance 2 Node 测试 6 项、双语检查 47 项通过。Zepp 全运动汇总入口的历史分页已成功获取，`zepp-sync-v6` 账本证明晨报前七天完整；worker 写锁修复后，今日自动晨报按正式去重标记投递，`prior_7d_verified=true`，未额外发送测试推送。旧训练详情的批量补采因服务器内存与 swap 压力暂停，精确遗留尝试已取消且数据库再次检查正常；未核验的动作代码和供应商无法提供的 App 校正名称仍保持未知，不声称全部明细或动作字典已经查全。
...
- 当前工作分支为 `fix/zepp-coverage-readable-reports`；英文 sidecar 仍只用于文档对照，不改变运行时报告路由。
- 旧版 2026-09-24 的历史基线：晨报调度曾返回 `stored_data_incomplete`：通用历史入口成功，而其余运动分类入口不可用，前七天训练历史未获完整证明。事实版不会把不可用解释为空记录，也不会修改历史覆盖标记。服务器备份和 schema 审计通过，未执行结构迁移。
...
- [ ] `AUD-017` **P2 / 力量动作证据**。已增加同场核验的有限显示名映射；Zepp 已检查的详情接口没有返回完整动作目录，App 修正组来源仍未确认，低内存服务器的历史明细补采因写锁而暂停。未知 code 不泛化，`unit` 和未映射动作的 `name` 仍未知。完成：取得同设备同训练的脱敏对照或实际 App 目录，验证映射、来源优先级与剩余历史详情。

...
- [ ] `AUD-017` **P2 / strength exercise evidence**. A limited set of same-workout verified display labels was added. The inspected Zepp detail endpoint did not return a complete movement catalog, the source of app-corrected sets remains unverified, and historical detail backfill on the low-memory server was paused after write-lock pressure. Unknown codes must not be generalized; `unit` and unmapped exercise `name` remain unknown. Done with redacted same-device/same-workout or actual App-catalog evidence verifying mappings, source precedence, and remaining historical details.

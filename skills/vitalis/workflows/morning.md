...
事实版（`facts_only`）只展示返回的睡眠、身体状态和必要限制；不得补写训练动作、强度、重量或从完整日报重建处方。完整处方版按以下顺序渲染：
...
4. 当 `decision_action` 为 `INSUFFICIENT_DATA` 时，只说明 `data_quality`、`key_reasons` 和 `cautions`，不得改用昨天数据、通用训练建议或补偿性安排。
